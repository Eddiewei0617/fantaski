...
function Boots({ toggleState, clickToChangeToggle, handleAddNumber }) {
  let storage = localStorage;
  const [boots, setBoots] = useState([]);
  useEffect(async () => {
    let res = await axios.get("http://localhost:3001/api/products/boots");
    setBoots(res.data);
  }, []);

  return (
    <>
      <h3 className="product_title pl-1">{CATEGORY_WORD[4]}</h3>
